...
You can find the `Dsuite` program [here](https://github.com/millanek/Dsuite). We only used the Loch Lomond population from Scotland, and Red Tarn from England, and excluded the Russian population due to high missing data. We remove the unwanted samples and create the `reduced.recode.vcf` file. We also subsample the ML tree obtained from RAxML for the Dsuite analysis.
